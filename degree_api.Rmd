...
pr("degree_api_fixed.R") %>% pr_run(port = 8000)
```
```{r}
...
file_content <- readLines("C:/Users/UTC/OneDrive/Bureau/database/degree_api.Rmd ", encoding = "UTF-8")
writeLines(file_content, "degree_api_fixed.R", useBytes = TRUE)# convert file in the path
pr("degree_api_fixed.R") %>% pr_run(port = 8000)
```
```{r}
# Test if file can be sourced normally
source("C:/Users/UTC/OneDrive/Bureau/database/degree_api_fixed.R ", encoding = "UTF-8")
```
```{r}
pr("C:/Users/UTC/OneDrive/Bureau/database/degree_api_fixed.R")  # Original problematic path
```
```{r}
# 1. Identify problematic file
problem_file <- "C:/Users/UTC/OneDrive/database/Temp/RtmpWYmEUF/filed4c66241291"

# 2. Create fixed version in working directory
fix_encoding <- function(src) {
  content <- readLines(src, encoding = "UTF-8")
  writeLines(content, "api_fixed.R", useBytes = TRUE)
  return("api_fixed.R")
}

# 3. Use the clean copy
pr(fix_encoding(problem_file)) %>% pr_run(port = 8000)
```
